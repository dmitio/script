...
if [[ "${release}" == "arch" ]]; then
    echo "您的操作系统为 Arch Linux"
elif [[ "${release}" == "parch" ]]; then
    echo "您的操作系统为 Parch Linux"
elif [[ "${release}" == "manjaro" ]]; then
    echo "您的操作系统为 Manjaro"
elif [[ "${release}" == "armbian" ]]; then
    echo "您的操作系统为 Armbian"
...
config_after_install() {
    ...
    if [[ "${config_confirm}" == "y" || "${config_confirm}" == "Y" ]]; then
        ...
        if [[ ! -f "/etc/x-ui/x-ui.db" ]]; then
            ...
            echo -e "这是一次全新的安装，出于安全考虑，将生成随机登录信息："
            echo -e "###############################################"
            echo -e "${green}用户名:${usernameTemp}${plain}"
            echo -e "${green}密码:${passwordTemp}${plain}"
            echo -e "###############################################"
            echo -e "${red}如果您忘记了登录信息，您可以在安装后键入x-ui，然后键入8进行查看${plain}"
        else
            echo -e "${red} 这是您的升级，将保留旧设置，如果您忘记了登录信息，您可以键入x-ui，然后键入8进行查看${plain}"
        fi
    fi
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red}获取 x-ui 版本失败，可能是由于Github API限制，请稍后再试${plain}"
            exit 1
        fi
        ...
        if [[ $? -ne 0 ]]; then
            echo -e "${red}下载 x-ui 失败，请确保您的服务器可以访问Github${plain}"
            exit 1
        fi
    else
        last_version=$1
    ...
    echo -e "${green}x-ui ${last_version}${plain} 安装完成，现在正在运行..."
    echo -e ""
    echo -e "x-ui 控制菜单用法: "
    echo -e "----------------------------------------------"
    echo -e "x-ui              - 进入     管理菜单"
    ...
    echo -e "x-ui log          - 检查     x-ui 日志"
    echo -e "x-ui banlog       - 检查 Fail2ban 封禁日志"
